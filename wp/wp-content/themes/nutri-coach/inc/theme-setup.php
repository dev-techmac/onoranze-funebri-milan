...
class ThemeSetup {
	public function __construct() {
		...
		add_filter( 'nutri_coach_logger_data', array( $this, 'provide_tracking_data' ) );
		add_filter(
			'nutri_coach_sdk_enable_logger',
			function ( $enabled ) {
				return 'yes';
			}
		);
	...
	public function add_product( $product ) {
		$product[] = NUTRI_COACH_THEME_FILE;
		return $product;
	}
	...
	private function get_anonymous_site_id() {
		$site_id = get_option( 'nutri_coach_site_id' );

		if ( ! $site_id ) {
			$site_id = wp_generate_password( 32, false );
			update_option( 'nutri_coach_site_id', $site_id );
		}
	...
	private function get_install_date() {
		$install_date = get_option( 'nutri_coach_install_date' );

		if ( ! $install_date ) {
			$install_date = current_time( 'Y-m-d' );
			update_option( 'nutri_coach_install_date', $install_date );
		}
	...
	private function is_usage_allowed() {
		$tracking_allowed = get_option( 'nutri_coach_allow_tracking', false );
		return apply_filters( 'nutri_coach_allow_tracking', $tracking_allowed );
	}
}
